...
export interface DitherOptions {
  ...
  /** invert: draw ink where bright instead of dark */
  invert?: boolean;
  /** temporal offset for shimmering animation */
  phase?: number;
}

/**
 * Dithers `src` (read as luminance) into `dst` as ink-on-transparent.
...
export function ditherLuminance(
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      // luminance 0..1 (premultiplied against white so transparent = white)
      let lum =
        (0.2126 * s[i]! + 0.7152 * s[i + 1]! + 0.0722 * s[i + 2]!) / 255;
