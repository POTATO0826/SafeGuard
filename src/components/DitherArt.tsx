...
interface DitherArtProps {
  ...
  pixelSize?: number;
  animate?: boolean;
  /** blue ink on white (default) or white ink for dark panels */
  whiteInk?: boolean;
  /** slow the animation clock */
  timeScale?: number;
}
...
export default function DitherArt({
  ...
  useEffect(() => {
    ...
    const setup = () => {
      // clientWidth/Height are layout sizes, immune to transform: scale()
      // (entry animations start scaled to 0, which would zero a rect measure)
